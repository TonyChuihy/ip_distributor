...
const clientId = `client_${Math.random().toString(36).substr(2, 6)}`; //非唯一ID，僅用在clint.js中控制臺調試
...
wsLB.on("open", () => {
  ...
  wsLB.send(
    JSON.stringify({
      type: "register",
      service: clientConfig.serviceType,
      // clientId: clientId,
    })
  );
});
...
wsLB.on("message", (data) => {
  ...
  if (message.type === "assigned") {
    console.log(`[Client ${clientId}] 分配到后端: ${message.backend.uid}`);
